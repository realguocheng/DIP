function [F,V] = MarchingCubes2(v,s)
...
for i=1:12
    ...
    pl(t_ida, 1:4, i) = [p (1:size(t_idb, 1))' + indexCount];
...
tri = table2(cube_code(t_id)+1, :);
for i=1:3:15
    idt = find(tri(:, i) > 0);
    T = [idt, 4*ones(size(idt, 1), 1),tri(idt, i:i+2) ];
    if ~isempty(T)
        p1 = sub2ind(size(pl), T(:,1), T(:,2), T(:,3));
        p2 = sub2ind(size(pl), T(:,1), T(:,2), T(:,4));
        p3 = sub2ind(size(pl), T(:,1), T(:,2), T(:,5));
